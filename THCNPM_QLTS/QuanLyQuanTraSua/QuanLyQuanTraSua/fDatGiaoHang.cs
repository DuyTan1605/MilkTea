...
{
    public partial class fDatGiaoHang : Form
    {
        public fDatGiaoHang()
        {
            InitializeComponent();
        ...
        private void FDatGiaoHang_Load(object sender, EventArgs e)
        {
            DataTable x = DataProvider.Instance.ExcuteQuery("SELECT ID_HoaDon FROM SHIP WHERE ID_Shipper IS  NULL");
            int[] res = new int[100];
            int dem = 0;
            ...
            DataTable data = DataProvider.Instance.ExcuteQuery("SELECT * FROM SHIP WHERE ID_Shipper IS  NULL");
            int S = 0;
