...
namespace QuanLyQuanTraSua
{
    public partial class fBaoCaoNgay : Form
    {
        ...
        {
            DateTime date = Convert.ToDateTime(dtpDate.Value.ToString());
            Console.WriteLine("SELECT ID FROM HOADON WHERE NgayDat='" + date.Year + "-" + date.Month + "-"+ date.Day + "'");
            DataTable x = DataProvider.Instance.ExcuteQuery("SELECT ID FROM HOADON WHERE DATEDIFF(DAY,NgayDat,'"+date+"')=0");
            int[] res = new int[100];
            int dem = 0;
            ...
            DataTable data = DataProvider.Instance.ExcuteQuery("SELECT * FROM HOADON WHERE DATEDIFF(DAY,NgayDat,'" + date + "')=0");
            int S = 0;
        ...
        private void FBaoCaoNgay_Load(object sender, EventArgs e)
        {
            DtpDateOfBirth_ValueChanged(sender,e);
        }
    }
}
