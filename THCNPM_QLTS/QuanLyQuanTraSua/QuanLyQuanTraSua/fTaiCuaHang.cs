...
{
    public partial class fTaiCuaHang : Form
    {
        public fTaiCuaHang()
        {
            InitializeComponent();
        ...
        private void FTaiCuaHang_Load(object sender, EventArgs e)
        {
            DataTable x = DataProvider.Instance.ExcuteQuery("SELECT ID FROM HOADON WHERE KT_Ship=0");
            int[] res = new int[100];
            int dem = 0;
            ...
            DataTable data = DataProvider.Instance.ExcuteQuery("SELECT * FROM HOADON WHERE KT_Ship=0");
            int S = 0;
